...
class AccountInvoice(models.Model):
    ...
    type = fields.Selection(selection_add=[('out_paying', 'sale_paying'), ('in_paying', 'purchase_paying')])
            ...
            for payment in self.payment_move_line_ids:
                ...
                if self.type in ('out_invoice', 'in_refund', 'out_paying'):
                    ...
                    if payment.matched_debit_ids:
                        payment_currency_id = all([p.currency_id == payment.matched_debit_ids[0].currency_id for p in payment.matched_debit_ids]) and\
                                              payment.matched_debit_ids[0].currency_id or False
                    ...
                    if payment.matched_credit_ids:
                        payment_currency_id = all([p.currency_id == payment.matched_credit_ids[0].currency_id for p in payment.matched_credit_ids]) \
    ...
    def refund(self, date_invoice=None, date=None, description=None, journal_id=None):
        ...
        for invoice in self:
            ...
            invoice_type = {'out_invoice': _('customer invoices refund'),
                            'in_invoice': _('vendor bill refund'),
                            'out_paying': _('customer invoices paying refund'),
                            'in_paying': _('vendor invoices paying refund')}
    ...
    def pay_and_reconcile(self, pay_journal, pay_amount=None, date=None, writeoff_acc=None):
        ...
        """
        if isinstance(pay_journal, (int, float)):
